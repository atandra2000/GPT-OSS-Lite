...
`models/attention.py:causal_attention` then runs SDPA with the windowed/full mask and the clamped sink bias (`sink_bias_cache` in `generate` memoizes the clamp per layer across steps). Correctness properties pin this behavior: the causal mask and sliding-window mask align query global indices $\text{idx}_q = \text{arange}(T_q) + (T_k - T_q)$ against key global indices $\text{idx}_k = \text{arange}(T_k)$. For any query/key dimensions $(T_q, T_k)$ — including single-token decode ($T_q=1$), standard prefill ($T_q=T_k$), or multi-token chunk prefill into a cached prefix ($1 < T_q < T_k$) — attention correctly permits keys where $\text{idx}_q \ge \text{idx}_k$ and $\text{idx}_q - \text{idx}_k < \text{window}$, avoiding future-token leakage across arbitrary sequence boundaries.
...
**Mask correctness across prefill, decode, and chunk prefill.** Regression tests pin the fixed mask behavior: `test_causal_attention_chunk_prefill_cached_prefix_is_causal` (verifies $1 < T_q < T_k$ chunk prefill into a cached context does not leak future tokens within the chunk), `test_sliding_window_blocks_past_keys_at_prefill` (the `_window_mask` branch), and `test_sink_path_matches_manual_at_prefill` (the additive `torch.where` sink mask matches `models/attention.py:manual_causal_attention`).
