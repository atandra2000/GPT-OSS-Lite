...
> primer level of [foundations](foundations-and-architecture.md) §2–§4; the sink mechanism is
> treated in depth in [ATTENTION_SINKS](attention-sinks.md) and the KV-cache
> consequences in [kv cache engineering](../inference.md).
...
- **Learned sink bias.** Each of the 8 heads carries one scalar logit that acts
  ...
  mathematics is derived in [ATTENTION_SINKS §4](attention-sinks.md), and the
  BF16 clamp rationale in [ATTENTION_SINKS §6](attention-sinks.md). This
...
- **GQA.** 8 query heads share 4 KV heads (`head_dim = 96`). The broadcast in
  ...
  ([ATTENTION_SINKS §8](attention-sinks.md), `scripts/kv_cache_benchmark.py`).
...
is derived in [ATTENTION_SINKS §8](attention-sinks.md) and measured by
...
of [ATTENTION_SINKS §4.3](attention-sinks.md); §5.2 shows how the code
...
4K→128K; see [rope_yarn](attention-and-positional.md)).
...
> companion is [rope_yarn](attention-and-positional.md) (worked numbers, dtype contract,
> SDPA interaction); this chapter supplies the derivations behind those
> numbers. Assumes the primer level of [foundations](foundations-and-architecture.md); the
...
- **Rotated K in the KV cache.** Keys are rotated *before* they enter
  ...
  math is in [kv cache engineering](../inference.md).
- **Budget honesty.** The ≥85% passkey @128K figure is a **target**; no
...
Scaled dot-product attention (derived in [attention math](attention-and-positional.md))
...
interacts with ([ATTENTION_SINKS](attention-sinks.md)).
...
(even = SWA, odd = full; [ATTENTION_SINKS](attention-sinks.md)) thus
...
several dims ([rope_yarn §10.2](attention-and-positional.md) reads it the other way).
...
([kv cache engineering](../inference.md), `scripts/kv_cache_benchmark.py`).
...
> Attention consumer: [attention-sinks.md](attention-sinks.md#part-b--implementation-modelsattentionpy).
