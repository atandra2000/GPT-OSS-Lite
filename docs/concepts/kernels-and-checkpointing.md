...
> Related: [training](../training.md), [architecture](foundations-and-architecture.md),
> [foundations](foundations-and-architecture.md), [attention math](attention-and-positional.md),
> [numerics](optimizers-and-numerics.md), [kv cache](../inference.md).
...
- **MoE feed-forward.** Each layer routes each token to top-2 of 8 experts plus
  1 shared expert (see [MoE theory](moe.md)). The estimator in
...
- **128K eval target.** Eval at 131,072 tokens runs under `torch.no_grad` —
  ...
  story (the measured 2.00× mixed-cache reduction, [attention sinks](attention-sinks.md),
...
([attention math](attention-and-positional.md)). Gradient checkpointing is that principle
...
$\beta = 2$ B/elt under BF16 autocast ([numerics](optimizers-and-numerics.md)). Weights and
...
- **Silent no-op when grad is disabled.** `torch.is_grad_enabled()` is part of
  ...
  with checkpointing — eval memory is KV-cache-dominated ([kv cache](../inference.md)).
